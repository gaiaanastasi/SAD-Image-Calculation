...
nBits = 8
outBits = 16
fname = "C:\Users\ZenBook Pro\Documents\GitHub\SAD-Image-Calculation\tb\sad_tb_2.vhdl"
finish = 2**(nPixel-1)+2
stop = 2**(nBits) -1
...
out_file.write("nPixel : positive :=" + str(nPixel) + ";\n")
out_file.write("nBits : positive :=" + str(nBits) + ";\n")
out_file.write("outBits : positive :=" + str(outBits) + ";\n")
out_file.write(");\n")
out_file.write("  port (\n")
out_file.write("    pixel_A		: in std_logic_vector("+ str(nBits-1) + " downto 0);\n")
out_file.write("    pixel_B	    : in std_logic_vector(" + str(nBits-1) + " downto 0);\n")
out_file.write("    clk		    : in std_logic;\n")
out_file.write("    rst		    : in std_logic;\n")
out_file.write("    enable		: in std_logic;	\n")
out_file.write("    sad		    : out std_logic_vector(" + str(outBits-1) + " downto 0);\n")
out_file.write("    valid	    : out std_logic	;\n")
out_file.write("    new_comp    : in std_logic;\n")
out_file.write("    count       : out std_logic_vector(" + str(nPixel-1)+ " downto 0)\n")
out_file.write("  );\n")
...
out_file.write("signal pixel_A_ext : std_logic_vector (" + str(nBits-1) + " downto 0) := (others => '0');\n")
out_file.write("signal pixel_B_ext : std_logic_vector (" + str(nBits-1) + " downto 0) := (others => '0');\n")
out_file.write("signal valid_ext : std_logic;\n")
out_file.write("signal sad_ext : std_logic_vector (" + str(outBits-1) + " downto 0);\n")
out_file.write("signal testing : boolean := true;\n")
out_file.write("signal new_comp_ext: std_logic:= '1';\n")
out_file.write("signal count_ext : std_logic_vector(" + str(nPixel-1) + "downto 0);\n")
...
out_file.write("    nPixel =>" + str(nPixel) + ",\n")
out_file.write("    nBits =>" + str(nBits) + ",\n")
out_file.write("    outBits =>" + str(outBits) + "\n")
out_file.write(")\n")
...
for a in range(finish):
	...
	pixelA = randint(0, stop)
    pixelB = randint(0, stop)
    out_file.write("    pixel_A_ext <= \"" +str(bin(pixelA)) + "\"; \n" )
    out_file.write("    pixel_B_ext <= \"" +str(bin(pixelB)) + "\"; \n" )
